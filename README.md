...
EBlink ARM Cortex-M debug tool with squirrel scripting device support
 ...
 ![ Windows installer ](https://www.embitz.org) **The installer always has the latest script commits.** Just download and use the latest installer, which will uninstall the current if needed, to update EBlink at the same installation location as the previous.
